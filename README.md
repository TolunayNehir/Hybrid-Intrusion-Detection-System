...
## Generated Files and GitHub Cleanup

Before publishing this project as a clean GitHub repository, remove local runtime and generated files.
...
```bash
rm -f storage/training_history.db
rm -f storage/*.db-shm
rm -f storage/*.db-wal
rm -f data/last_train_metrics.json
rm -f data/model_train_status.json
```

Optional: remove generated checkpoints from the public repository:
...
Optional: remove unrelated attached assets:
